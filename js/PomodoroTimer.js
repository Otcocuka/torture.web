export class PomodoroTimer {
    ...
    updateStatsDisplay() {
        ...
        this.totalBreak.textContent = this.formatDuration(
            this.user.stats.totalBreak
        );
        this.totalPaused.textContent = this.formatDuration(
            this.user.stats.totalPaused
        );
    }
    ...
    addEventListeners() {
        ...
        this.workInput.addEventListener("input", (e) =>
            this.updateSettings("work", e.target.value)
        );
        this.shortBreakInput.addEventListener("input", (e) =>
            this.updateSettings("short", e.target.value)
        );
        this.longBreakInput.addEventListener("input", (e) =>
    ...
    pause() {
        if (this.isRunning) {
            ...
            const pausedDuration = Math.floor(
                (Date.now() - this.user.stats.lastPauseTime) / 1000
            );
    ...
    resetStats() {
        this.user.settings = { work: 25, short: 5, long: 15, longCycle: 4 };
        this.user.stats = {
            totalSessions: 0,
            totalWork: 0,
            totalBreak: 0,
    ...
    exportData() {
        return {
            ...
            dopamineHistory:
                JSON.parse(localStorage.getItem("dopamineWheelHistory")) || [],
        };
    }
}
