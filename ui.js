...
const UI = {
    init() {
        ...
        document
            .querySelector('[data-nav="view-habits"]')
    ...
    bindHabitsEvents() {
        ...
        document.getElementById("habitSettingsBtn").onclick = () =>
            this.showHabitSettingsModal();
        document.getElementById("viewAchievementsBtn").onclick = () =>
    ...
    renderHabits() {
        ...
        if (habits.length === 0) {
            container.innerHTML =
                '<div class="text-center text-gray-400 py-8">Нет привычек. Добавьте первую!</div>';
            return;
        }
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-${color}-500 animate-fade-in">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-xs bg-gray-100 px-2 py-1 rounded">${h.count
                    } дн.</span>
                            <button class="delete-btn text-gray-300 hover:text-red-500 text-lg px-2" data-id="${h.id
                    }">×</button>
                        </div>
                    </div>
                    ${h.subtasks?.length
                            ...
                            .map(
                                ...
                                <label class="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                                    <input type="checkbox" class="rounded text-${color}-500"
                                        data-action="toggle-subtask" data-habit-id="${h.id
                                    }" data-subtask-id="${st.id}"
                                        ${st.completed ? "checked" : ""}>
                                    <span class="${st.completed
                                        ? "line-through text-gray-400"
                                        : ""
                                    }">${st.text}</span>
                                    <span class="text-xs text-gray-400">(${st.dates?.length || 0
                                    })</span>
                                </label>
                            `
                            )
                    ...
                    <button data-action="increment" data-id="${h.id
                    }" class="w-full mt-2 py-1.5 rounded text-sm bg-${color}-100 text-${color}-800 hover:bg-${color}-200 font-medium">
                        Отметить выполнение
                    </button>
                    ${h.history.length
                        ? `
                        <details class="mt-2 text-xs text-gray-500">
                            <summary class="cursor-pointer hover:text-blue-500">История (${h.history.length
                        })</summary>
                            ...
                            .map(
                                (hi) => `
                                    <div>${new Date(
                                    hi.date
                                ).toLocaleDateString()}: ${hi.subtasks.length
                                    } подп.</div>
                                `
                            )
                            .join("")}
    ...
    showHabitSettingsModal() {
            ...
            <div class="bg-white rounded-lg p-6 w-80 shadow-xl">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input id="hGoal" type="number" value="${s.goal
            }" class="w-full border p-2 rounded">
                    </div>
                    <div>
                        <label class="text-sm block mb-1">Цвет</label>
                        <select id="hColor" class="w-full border p-2 rounded">
                            <option value="blue" ${s.color === "blue" ? "selected" : ""
            }>Синий</option>
                            <option value="green" ${s.color === "green" ? "selected" : ""
            }>Зеленый</option>
                            <option value="purple" ${s.color === "purple" ? "selected" : ""
            }>Фиолетовый</option>
                        </select>
                    </div>
        ...
        const renderTemp = () => {
            ...
            if (!list) return;
            list.innerHTML = Store.data.tempSubtasks
                .map(
                    (s) => `
    ...
    showAchievementsModal() {
        ...
        const html = list.length
            ? list
                .map(
                    (a) =>
                        `<div class="flex justify-between bg-green-50 p-2 rounded"><span>${a.name}</span><span class="font-bold">x${a.goal}</span></div>`
                )
                .join("")
    ...
    bindTimerEvents() {
        ...
        if (saveSetBtn) {
            saveSetBtn.onclick = () => {
                const s = {
                    ...
                    longCycle:
                        parseInt(document.getElementById("settingCycle").value) || 4,
                };
        ...
        if (startBtn) {
            startBtn.addEventListener(
                "click",
                () => window.Controllers.pomodoro.requestNotificationPermission(),
                { once: true }
            );
        }
    },
    ...
    updateTimerDisplay(mins, secs, isWorking) {
        ...
        if (isWorking) {
            ...
            if (container)
                container.className =
                    "bg-white p-8 rounded-2xl shadow-lg text-center mb-6 transition-standard theme-work";
        } else {
            if (phase) phase.textContent = "ОТДЫХ";
            if (container)
                container.className =
                    "bg-white p-8 rounded-2xl shadow-lg text-center mb-6 transition-standard theme-rest";
        }
    },
    ...
    bindTodoEvents() {
        // Добавление колонки
        document
            .getElementById("addKanbanColBtn")
        ...
        // Обработка кликов внутри доски (делегирование)
        ...
        if (board) {
                ...
                if (btn.dataset.action === "add-card") {
                    ...
                    document.getElementById("saveKanbanCard").onclick = () => {
                        const title = document
                            .getElementById("kanbanCardTitle")
                            .value.trim();
            ...
            board.addEventListener("drop", (e) => {
                ...
                if (cardId && column) {
                    ...
                    const oldColId = oldCardElement
                        ? parseInt(oldCardElement.dataset.columnId)
                        : 0;
    ...
    renderKanban() {
        ...
        if (!data.columns.length) {
            board.innerHTML =
                '<div class="text-gray-400 p-4">Нет колонок. Добавьте первую!</div>';
            return;
        }
            ...
            .map((col) => {
                ...
                <div class="flex-shrink-0 w-72 bg-gray-100 rounded-lg p-2 flex flex-col max-h-[600px]">
                    <div class="flex justify-between items-center mb-2 bg-white p-2 rounded shadow-sm">
                        <h3 class="font-bold text-gray-700 truncate">${col.title
                    }</h3>
                        <div class="flex gap-1">
                            <button data-action="add-card" data-column-id="${col.id
                    }" class="text-green-600 font-bold text-xl hover:text-green-800" title="Добавить задачу">+</button>
                            <button data-action="delete-column" data-column-id="${col.id
                    }" class="text-red-300 font-bold hover:text-red-500">×</button>
                        </div>
                    </div>
                    <div class="kanban-column-inner flex-1 overflow-y-auto space-y-2 p-1" data-column-id="${col.id
                        ...
                        ? cards
                            ...
                            <div class="kanban-card bg-white p-3 rounded shadow-sm border-l-4 border-blue-400 cursor-move hover:shadow-md transition-shadow" 
                                ...
                                <div class="flex justify-between items-start mb-1">
                                    <span class="font-semibold text-sm text-gray-800">${card.title
                                    }</span>
                                    <button data-action="delete-card" data-card-id="${card.id
                        ...
                        `
                            ...
                            .join("")
                        : '<div class="text-xs text-gray-400 text-center py-2">Пусто</div>'
                    }
                    </div>
                </div>
            `;
    ...
    showNotification(msg) {
        ...
        n.className =
            "fixed top-4 left-1/2 -translate-x-1/2 bg-gray-800 text-white px-4 py-2 rounded-lg shadow-lg animate-fade-in z-[60]";
        n.textContent = msg;
        document.body.appendChild(n);
        setTimeout(() => n.remove(), 3000);
    },

    // --- STATS VIEW (Updated with Uptime and Wheel Fix) ---
    renderStatsView() {
        // 1. Pomodoro
        const p = Store.data.pomodoro.stats;
        const s1 = document.getElementById("stat-t-sessions");
        if (s1) s1.textContent = p.totalSessions;
        const s2 = document.getElementById("stat-t-work");
        if (s2) s2.textContent = this.formatDuration(p.totalWork);
        const s3 = document.getElementById("stat-t-break");
        if (s3) s3.textContent = this.formatDuration(p.totalBreak);
        const s4 = document.getElementById("stat-t-paused");
        if (s4) s4.textContent = p.totalPaused + "с";
        ...
        if (mContainer) {
            let mHtml = "";
            for (let i = 0; i < 12; i++)
                mHtml += `<span style="width:30px">${i + 1}</span>`;
        ...
        if (hCont) {
            if (habits.length === 0) {
                hCont.innerHTML =
                    '<div class="text-gray-500 text-sm">Нет активных привычек</div>';
            } else {
                hCont.innerHTML = habits
                    .map((habit) => {
    ...
    bindGlobalEvents() {
        ...
        document.getElementById("btnExport")?.addEventListener("click", (e) => {
            ...
            a.href = url;
            a.download = `torture2_backup_${new Date()
                .toISOString()
                .slice(0, 10)}.json`;
    ...
    flashTitle(message) {
        ...
        this.titleInterval = setInterval(() => {
            // Меняем между оригиналом и сообщением
            document.title = on ? this.originalTitle : `⚠️ ${message} ⚠️`;
            on = !on;
        }, 500); // Быстрее, каждые 0.5 сек
    ...
    bindNotificationsEvents() {
        ...
        document.getElementById("addNotifBtn")?.addEventListener("click", () => {
            ...
            if (title && interval) {
                ...
                // Запрашиваем разрешение на уведомления, если еще нет
    ...
    renderNotificationsList() {
        ...
        container.innerHTML = list.map(n => {
            ...
            if (n.wasClicked) {
                ...
                // Только что сработало, ждем ответа
                ...
                // Прошло больше интервала, чем нужно - считаем пропущенным
