...
const UI = {
                ...
                <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-${color}-500 animate-fade-in">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button class="delete-btn text-gray-300 hover:text-red-500 text-lg px-2" data-id="${
                              h.id
                            }">×</button>
                        </div>
                    </div>
                              ...
                              .map(
                                ...
                                <label class="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
                                    <input type="checkbox" class="rounded text-${color}-500"
                                        data-action="toggle-subtask" data-habit-id="${
                                          h.id
                    ...
                    <button data-action="increment" data-id="${
                      h.id
                    ...
                    ${
                      h.history.length
                        ...
                        <details class="mt-2 text-xs text-gray-500">
                            <summary class="cursor-pointer hover:text-blue-500">История (${
                              h.history.length
                            })</summary>
            ...
            <div class="bg-white rounded-lg p-6 w-80 shadow-xl">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input id="hGoal" type="number" value="${
                          s.goal
                    ...
                    <div>
                        ...
                        <select id="hColor" class="w-full border p-2 rounded">
                            <option value="blue" ${
                              s.color === "blue" ? "selected" : ""
                            }>Синий</option>
                            <option value="green" ${
                              s.color === "green" ? "selected" : ""
                            }>Зеленый</option>
                            <option value="purple" ${
                              s.color === "purple" ? "selected" : ""
                            }>Фиолетовый</option>
                        </select>
                    </div>
    ...
    if (btn)
      btn.onclick = () => {
        if (window.Controllers && window.Controllers.wheel) {
          window.Controllers.wheel.spin(); // Просто запускаем, Promise здесь не обязателен для UI, но логика сохранения надежна
        }
      };
  },
  ...
  renderStatsView() {
    ...
    // 2. Uptime (NEW) - Защита от ошибок
    const uEl = document.getElementById("stat-total-uptime");
    if (uEl) {
      let uptimeSecs = 0;
      // Проверяем наличие функции и данных
      if (typeof Store.getAppUptime === "function") {
        uptimeSecs = Store.getAppUptime();
      }
      // Добавляем текущую сессию (не сохраненную еще в Store)
